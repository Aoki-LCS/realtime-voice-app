...
- **環境変数による厳格なキー管理**: APIキーは画面やフロントエンドコード内に直接入力・保持させず、`.env` ファイルまたはホスティング環境（Vercel 等）の環境変数 `GEMINI_API_KEY` から読み込みます。
- **未設定時の安全停止**: `GEMINI_API_KEY` が設定されていない（または空の）場合、通信を開始せず明示的なエラーメッセージを表示して安全に停止します。
