...
- **ユーザーごとのAPIキー設定 (BYOK方式)**: 利用者がブラウザ上の設定モーダルから自身の Gemini API キーを入力・保持して対話できる機能を搭載。キーは各ユーザーのブラウザ内（localStorage）にのみ保持され、作者のAPI枠や費用を消費することなく安全にシェア・実行が可能です。
- **環境変数のフォールバック**: サーバー側に `GEMINI_API_KEY` が設定されている場合はデフォルトで使用し、未設定の場合は自動的にユーザー入力モーダルが表示されます。
